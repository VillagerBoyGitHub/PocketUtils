...
public function setExperienceAndLevel(int$exp,int$level){trigger_error("");
return$this->setTotalXp(self::getTotalXpRequirement($level)+$exp);
}
public function setExp(int$exp){trigger_error("");
return$this->setTotalXp($exp);
}
public function setExpLevel(int$level){trigger_error("");
return$this->setXpLevel($level);
}
public function getExpectedExperience(){trigger_error("");
return self::getTotalXpRequirement($this->getXpLevel()+1);
}
public function getLevelUpExpectedExperience(){trigger_error("");
return self::getLevelXpRequirement($this->getXpLevel()+1);
}
public function calcExpLevel(){trigger_error("");
}
public function addExperience(int$exp){trigger_error("");
return$this->addXp($exp);
}
public function addExpLevel(int$level){trigger_error("");
return$this->addXpLevel($level);
}
public function getExp(){trigger_error("");
return$this->getTotalXp();
}
public function getExpLevel(){trigger_error("");
return$this->getXpLevel();
}
public function canPickupExp():bool{trigger_error("");
return$this->canPickupXp();
}
public function resetExpCooldown(){trigger_error("");
$this->resetXpCooldown();
}
public function updateExperience(){trigger_error("");
}
...
if($this->inAirTicks < 300){
//Dica: 20 ticks = 1 seconds
//40 ticks = 2 seconds
//130 ticks = 15 seconds
//If player will fly for 15 seconds without any permission for fly, he got kick!
//You can turn it off at genisys.yml ;)
}elseif($this->kick("§eFly detected\nplease turn off your cheats, this is unfair!")){
...
protected function processLogin(){if(!$this->server->isWhitelisted(strtolower($this->getName()))){$this->close($this->getLeaveMessage(),"§eGET OUT! SERVER IS WHITE-LISTED!");
return;
}
...
foreach($this->server->getOnlinePlayers()as$p){if($p!==$this and strtolower($p->getName())===strtolower($this->getName())){if($p->kick("logged in from another location")===false){$this->close($this->getLeaveMessage(),"Logged in from another location");
return;
}
}
elseif($p->loggedIn and$this->getUniqueId()->equals($p->getUniqueId())){if($p->kick("logged in from another location")===false){$this->close($this->getLeaveMessage(),"Logged in from another location");
