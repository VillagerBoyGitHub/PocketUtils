...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer
{
    ...
    if ((strlen($packet->skin) != 64 * 64 * 4) and (strlen($packet->skin) != 64 * 32 * 4)) {
        ...
        break;
    } elseif ($packet->protocol == 84) {
        $this->setSkin($packet->skin, $packet->skinId);
    } else {
        if ($packet->someOtherCondition) {
            $this->setSkin($packet->skin, $packet->someOtherCondition);
        } else {
    ...
    if(!$ev->isCancelled()){$this->server->broadcastMessage($this->getServer()->getLanguage()->translateString($ev->getFormat(), [
